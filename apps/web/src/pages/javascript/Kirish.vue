<template>
  <div class="space-y-10 max-w-4xl pb-20 text-left">
    ...
    <nav class="flex items-center flex-wrap gap-2 text-xs font-mono text-surface-400">
      ...
      <span class="text-surface-600">&gt;</span>
      <span class="text-brand-400 font-bold">1.1. JavaScript & V8 Engine</span>
    </nav>

    <!-- Title & Meta -->
    <div class="space-y-4 border-b border-surface-800 pb-6">
      <h1 class="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
        JavaScript Kirish — Vebning Tirik Miyasi va V8 Dvigateli
      </h1>
      <p class="text-sm sm:text-base text-surface-300 leading-relaxed font-normal">
        JavaScript — sahifaga harakat, jonli interaktivlik, server bilan muloqot va hisob-kitoblar quvvatini beruvchi yagona nativ veb dasturlash tilidir.
      </p>
    ...
    <div class="p-6 rounded-2xl bg-surface-900/40 border border-surface-800/80 space-y-4">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2.5 text-amber-400 font-bold text-sm sm:text-base">
          ...
          <span>5 yoshli bola uchun tushuntirish: O‘yinchoq Robotning Yuragi</span>
        </div>
        <button
      ...
      <div v-if="!showUnderTheHood" class="space-y-2 text-xs sm:text-sm text-surface-300 leading-relaxed animate-in fade-in duration-200">
        <p>
          Lego dan robot yasadik (HTML), unga qizil kiyim kiydirdik (CSS). Lekin u hali qimirlamayapti.
        </p>
        <p class="pt-1">
          JavaScript — uning ichiga qo‘yilgan pultli batareyadir! Tugmani bossangiz, robot qo‘lini ko‘taradi va "Salom!" deb gapiradi.
        </p>
      </div>

      <div v-else class="space-y-2 font-mono text-xs animate-in fade-in duration-200">
        <div class="text-cyan-300 font-bold">V8 Engine C++: Bytecode Generation (Ignition Interpreter)</div>
        <pre class="p-3.5 rounded-xl bg-surface-950 border border-surface-800 text-cyan-300 text-[11px] leading-relaxed overflow-x-auto"><code>// v8/src/interpreter/interpreter.cc
void Interpreter::Execute(Handle&lt;BytecodeArray&gt; bytecode_array) {
  // JavaScript kodini to'g'ridan-to'g'ri protsessor registrlari orqali bajarish
  DispatchBytecode(bytecode_array-&gt;GetFirstBytecode());
}</code></pre>
      </div>
    </div>

    <!-- How JS Executes (Hard & Deep) -->
    <section class="space-y-4">
      <h2 class="text-xl sm:text-2xl font-bold text-white flex items-center gap-2">
        <span class="text-brand-400">#</span> 1. V8 Dvigateli JS ni Qanday Bajaradi?
      </h2>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs font-mono">
        <div class="p-4 rounded-xl bg-surface-900 border border-surface-800 space-y-2">
          <div class="text-brand-400 font-bold text-sm">1. Parser & AST</div>
          <p class="text-surface-300 text-[11px] font-sans leading-relaxed">
            Matn ko‘rinishidagi JS kodi Abstract Syntax Tree (AST) daraxtiga aylantiriladi.
          </p>
        </div>

        <div class="p-4 rounded-xl bg-surface-900 border border-surface-800 space-y-2">
          <div class="text-blue-400 font-bold text-sm">2. Ignition (Bytecode)</div>
          <p class="text-surface-300 text-[11px] font-sans leading-relaxed">
            V8 interpreteri AST ni tezkor baytkodlarga o‘girib, darhol ijro etishni boshlaydi.
          </p>
        </div>

        <div class="p-4 rounded-xl bg-surface-900 border border-surface-800 space-y-2">
          <div class="text-purple-400 font-bold text-sm">3. TurboFan (JIT)</div>
          <p class="text-surface-300 text-[11px] font-sans leading-relaxed">
            Ko‘p takrorlanuvchi funksiyalarni to‘g‘ridan-to‘g‘ri Mashina Kodiga (Machine Code) kompilyatsiya qiladi.
          </p>
        </div>
      </div>
    </section>
    ...
    <section class="space-y-4 pt-4">
      <h2 class="text-xl sm:text-2xl font-bold text-white flex items-center gap-2">
        <span class="text-brand-400">#</span> 2. Jonli Mashq: Birinchi JavaScript Funksiyangiz
      </h2>
