<template>
  <div class="space-y-10 max-w-4xl pb-20 text-left">
    ...
    <nav class="flex items-center flex-wrap gap-2 text-xs font-mono text-surface-400">
      ...
      <span class="text-surface-600">&gt;</span>
      <span class="text-brand-400 font-bold">2.2. DOM Manipulyatsiyasi</span>
    </nav>

    <!-- Title & Meta -->
    <div class="space-y-4 border-b border-surface-800 pb-6">
      <h1 class="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
        DOM Manipulyatsiyasi — Sahifani Real Vaqtda O‘zgartirish
      </h1>
      <p class="text-sm sm:text-base text-surface-300 leading-relaxed font-normal">
        DOM (Document Object Model) — JavaScript uchun HTML hujjatni obyektlar daraxti sifatida taqdim etuvchi ko‘prikdir. U orqali elementlarni qo‘shish, o‘chirish va stillarini o‘zgartirish mumkin.
      </p>
    ...
    <div class="p-6 rounded-2xl bg-surface-900/40 border border-surface-800/80 space-y-4">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2.5 text-amber-400 font-bold text-sm sm:text-base">
          ...
          <span>5 yoshli bola uchun tushuntirish: Sehrli Qo‘g‘irchoq Teatri</span>
        </div>
        <button
      ...
      <div v-if="!showUnderTheHood" class="space-y-2 text-xs sm:text-sm text-surface-300 leading-relaxed animate-in fade-in duration-200">
        <p>
          Qo‘g‘irchoq teatrida sahnadagi qahramonlar iplar orqali boshqariladi. Siz ipni tortsangiz (JavaScript), qahramon sakraydi yoki bosh kiyimini almashtiradi (DOM).
        </p>
      </div>

      <div v-else class="space-y-2 font-mono text-xs animate-in fade-in duration-200">
        <div class="text-cyan-300 font-bold">Blink V8 DOM Wrapper: V8HTMLHeadingElement Bindings</div>
        <pre class="p-3.5 rounded-xl bg-surface-950 border border-surface-800 text-cyan-300 text-[11px] leading-relaxed overflow-x-auto"><code>// third_party/blink/renderer/bindings/core/v8/v8_element.cc
void V8Element::InnerHTMLAttributeSetterCallback(...) {
  Element* element = V8Element::ToWrappableUnsafe(info.Holder());
  element-&gt;SetInnerHTML(v8_value); // C++ xotirasida DOM daraxtini qayta qurish
}</code></pre>
      </div>
    </div>

    <!-- DOM Best Practices (Hard & Deep) -->
    <section class="space-y-4">
      <h2 class="text-xl sm:text-2xl font-bold text-white flex items-center gap-2">
        <span class="text-brand-400">#</span> 1. Performans: Layout Thrashing dan Qochish
      </h2>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-mono">
        <div class="p-4 rounded-xl bg-red-950/20 border border-red-500/30 space-y-2">
          <div class="text-red-400 font-bold text-sm">❌ Noto‘g‘ri (Har qatorda Reflow)</div>
          <pre class="p-3 bg-surface-950 rounded-lg text-red-300 text-xs"><code>for(let i=0; i&lt;100; i++) {
  document.body.appendChild(
    document.createElement('div')
  ); // 100 marta reflow chaqiradi!
}</code></pre>
        </div>

        <div class="p-4 rounded-xl bg-emerald-950/20 border border-emerald-500/30 space-y-2">
          <div class="text-emerald-400 font-bold text-sm">✓ To‘g‘ri (DocumentFragment)</div>
          <pre class="p-3 bg-surface-950 rounded-lg text-emerald-300 text-xs"><code>const fragment = document.createDocumentFragment();
for(let i=0; i&lt;100; i++) {
  fragment.appendChild(document.createElement('div'));
}
document.body.appendChild(fragment); // Faqat 1 marta reflow!</code></pre>
        </div>
      </div>
    </section>
    ...
    <section class="space-y-4 pt-4">
      <h2 class="text-xl sm:text-2xl font-bold text-white flex items-center gap-2">
        <span class="text-brand-400">#</span> 2. Jonli Mashq: DOM Elementlarini Yaratish va Boshqarish
      </h2>
