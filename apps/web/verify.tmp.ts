import { coursesData, getCourseLessons, findLesson, topics } from './src/data/topics.ts';
import { getLesson, getNeighbours, courseThemes } from './src/data/lessonRegistry.ts';
import { COURSE_SLUGS } from './src/data/lessonTypes.ts';
...
for (const c of COURSE_SLUGS) {
  ...
  for (const l of lessons) {
    ...
    for (const f of ['description','spec','content','codeExample'] as const) {
      if (!d[f] || d[f].length < 40) bad(`${l.slug}: ${f} juda qisqa`);
    }
